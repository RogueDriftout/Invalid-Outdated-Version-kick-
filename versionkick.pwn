...
#include <zcmd> // the command processor
new bool:versioncheck, versionstring[10], playername[26], messagestring[80];// used to make the version controllable, put at the top of ur script.
	...
	{							// if its a player on a false version
		...
		{		//if the version kick is turned on
 			format(messagestring, sizeof(messagestring), " Server Has Kicked %s, Reason : OLD CLIENT VERSION %s ", playername,versionstring), SendClientMessageToAll(-1,messagestring);
   			SetTimerEx("DelayedKickAndMessages",500,false,"d",playerid);
   			}//delaying both the messages and the kick.
		else
...
{
    SendClientMessage(playerid,-1,".: OUTDATED VERSION, UPDATE FROM: sa-mp.com/download.php :."),SendClientMessage(playerid,-1,".: OR COPY DIRECT LINK FROM YOUR CHATLOG http://forum.sa-mp.com/files/testing/sa-mp-0.3.7-R2-install.exe :.");		//direct link & samp's link
	if(!versioncheck) SetPlayerWeather(playerid, 999), SetPlayerVirtualWorld(playerid, 999), SetPlayerInterior(playerid,999),Kick(playerid);			//you're free to replace the below lines to your desired samp version
	return 1;
}
